...
/**
 * Deadline in force for one dispatch: the per-call override when given,
 * otherwise the context's own setting. `0` disables it.
 * @param {!Object} context the context dispatching the command
 * @param {?Object} options per-call options, may be absent
 * @return {number} milliseconds, or 0 for no deadline
 */
const resolveTimeout = function(context, options) {
  if (options && options.timeout !== undefined && options.timeout !== null) {
    return options.timeout;
  }
  return context.commandTimeout || 0;
};

/**
 * Failure of a dispatched AGI command, naming the command it belongs to so a
 * caller can attribute it without parsing the message.
 * @param {?string} command the command as sent, without the trailing newline
 * @param {string} reason what went wrong
 * @return {!Error} the rejection
 */
const commandError = function(command, reason) {
  const err = new Error(
      'AGI command failed: ' + (command || 'unknown command') +
      ' — ' + reason);
  err.command = command || null;
  return err;
};
...
const Context = function(conn, loggerOptions = {}) {
  ...
  // Off by default, and deliberately so: `dial`, `recordFile`, `getData`,
  // `waitForDigit` and `streamFile` block by design -- an `EXEC Dial` stays
  // pending for the whole call -- so any fixed default eventually rejects a
  // live one. The caller picks a value above its longest blocking command.
  this.commandTimeout = loggerOptions.commandTimeout || 0;
  this.conn = conn;
  this.stream = conn;
...
Context.prototype.readVariables = function(msg) {
  ...
  lines.map(function(line) {
    // AGI sends `name: value`, and the value may itself contain colons —
    // `agi_request: agi://host/script`, or a timestamp argument. Splitting on
    // every colon truncated both, so split on the FIRST one only.
...
Context.prototype.readResponseLine = function(line) {
  ...
  if (!parsed) {
    // `511 Command Not Permitted on a dead channel` is the EXPECTED answer to
    // a channel read in the `h` extension. Returning here without touching
    // `pending` left that promise unsettled forever — silence the caller's
    // `.catch` could never see. Settle it first; the hangup still follows.
    if (this.pending) {
      const pending = this.pending;
      this.pending = null;
      pending(commandError(this.pendingCommand, line));
    }
...
Context.prototype.sendCommand = function(command, options) {
  if (this.debug) this.log('------->', {command: command});
  const self = this;
  const timeout = resolveTimeout(this, options);

  return new Promise(function(resolve, reject) {
    let timer = null;

    // Single exit for both routes, so an armed deadline is always disarmed --
    // a timer outliving its command would leak, and `mocha --exit` cannot see
    // that happen.
    ...
    if (timeout > 0) {
      timer = setTimeout(function() {
        timer = null;
        // Drop the callback so a late response cannot be mistaken for the
        // answer to whatever command runs next.
        if (self.pending === settle) self.pending = null;
        settle(commandError(command, 'no response after ' + timeout + 'ms'));
      }, timeout);
    }
...
Context.prototype.waitForDigit = function(timeoutIn) {
  const timeout = timeoutIn !== undefined && timeoutIn !== null ? timeoutIn : 5000;
